...
def pretrain_vae():
    ...
    vae_dataloader = DataLoader(protein_data, batch_size=2, shuffle=True, collate_fn=collate)
    vae_model = CodeBook(param, DataLoader(protein_data, batch_size=2, shuffle=False, collate_fn=collate)).to(device)
    vae_optimizer = torch.optim.Adam(vae_model.parameters(), lr=float(1e-4), weight_decay=float(param['weight_decay']))
    ...
    for epoch in range(1, param["pre_epoch"] + 1):
        for iter_num, batch_graph in enumerate(vae_dataloader):
            ...
            z, x_recon, recon_loss, mask_loss = vae_model(batch_graph)
            loss_vae = recon_loss + mask_loss * param['mask_loss']
            ...
            if (epoch - 1) % param['log_num'] == 0 and iter_num == 0:
                ...
                print("\033[0;30;43m Pre-training VQ-VAE | Epoch: {}, Batch: {} | Train Loss: {:.5f} | {:.5f} {:.5f}\033[0m".format(epoch, iter_num, loss_vae.item(), recon_loss.item(), mask_loss.item()))
                log_file.write("Pre-training VQ-VAE | Epoch: {}, Batch: {} | Train Loss: {:.5f} | {:.5f} {:.5f}\n".format(epoch, iter_num, loss_vae.item(), recon_loss.item(), mask_loss.item()))
                log_file.flush()
...
def main():
    ...
    vae_model = CodeBook(param, DataLoader(protein_data, batch_size=4, shuffle=False, collate_fn=collate)).to(device)
    ...
    prot_embed = vae_model.Protein_Encoder.forward().to(device)
    print("protein")
    ...
    model = HGNN(param).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=float(1e-3), weight_decay=float(param['weight_decay']))
    ...
    for epoch in range(1, param["max_epoch"] + 1):
        ...
        # 训练时传入 ppi_g_train
        train_loss, train_f1_score = train(model, ppi_g_train, prot_embed, ppi_list, labels, ppi_split_dict['train_index'], param['batch_size'], optimizer, loss_fn, epoch)
        
        scheduler.step(train_loss)

        if (epoch - 1) % param['log_num'] == 0:

            val_f1_score = evaluator(model, ppi_g_test, prot_embed, ppi_list, labels, ppi_split_dict['val_index'], param['batch_size'])
            test_f1_score = evaluator(model, ppi_g_test, prot_embed, ppi_list, labels, ppi_split_dict['test_index'], param['batch_size'])
    ...
    eval_output, eval_labels = evaluator(model, ppi_g_test, prot_embed, ppi_list, labels, ppi_split_dict['test_index'], param['batch_size'], 'output')
